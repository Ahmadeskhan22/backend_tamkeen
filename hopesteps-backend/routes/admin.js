...
router.get(
  "/dashboard",
  protect,
  authorize("admin", "donor", "volunteer"),
  async (req, res) => {
    try {
      const [
        totalStudents,
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!request)
        return res
          .status(404)
          .json({ status: "error", message: "الطلب غير موجود" });
...
router.get("/pledges", protect, authorize("admin"), async (req, res) => {
  try {
    ...
    res.json({ status: "success", count: pledges.length, data: pledges });
  } catch (err) {
    res.status(500).json({ status: "error", message: err.message });
  }
});
